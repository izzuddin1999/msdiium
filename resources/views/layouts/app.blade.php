<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" data-theme="iium-directory">
<head>
    <meta charset="utf-8">
    ...
    <style>
        :root { --flow-primary:#008f86; --flow-primary-dark:#087064; --flow-accent:#159cf5; --flow-dark:#202b39; --flow-soft:#f3f6f8; --flow-border:#dfe5ea; --flow-text:#344054; --flow-muted:#667085; --flow-shadow:0 8px 24px rgba(31,42,55,.07); }
    ...
    <style>
        /* Balanced IIUM palette: teal identifies the shell; content stays calm and neutral. */
        .flow-card,.flow-toolbar,.repository-results,.topic-governance-panel,.staff-directory-panel,.admin-stat-panel{border-color:#e1e6eb;box-shadow:var(--flow-shadow)}
        .metric-card>.material-icons,.results-heading .material-icons{background:#edf6ff;color:#1576bd}
        .metric-card.green>.material-icons{background:#e9f7ec;color:#278244}
        .metric-card.orange>.material-icons{background:#fff4df;color:#b77900}
        .metric-card.blue>.material-icons{background:#edf3ff;color:#376fc6}
        .admin-health-panel{background:linear-gradient(135deg,#1f2937,#344054)}
        .admin-kicker{color:#7dd3fc}
        .topic-panel-heading .topic-manage-button{border-color:#159cf5;background:linear-gradient(135deg,#159cf5,#087dcc);box-shadow:0 8px 18px rgba(21,156,245,.18)}
        .topic-panel-heading .topic-manage-button:hover{background:linear-gradient(135deg,#087dcc,#0667a8);box-shadow:0 10px 22px rgba(21,124,204,.23)}
        .topic-panel-heading .topic-manage-button:focus-visible{outline-color:rgba(21,156,245,.25)}
        .topic-dashboard-card{border-color:#e1e6eb;background:#fff}
        .topic-card-top{border-color:#e4e7ec}
        .topic-code{background:#edf6ff;color:#1576bd}
        .staff-directory-panel,.staff-msd-dashboard{background:#fff}
        .staff-unit-icon{background:linear-gradient(145deg,#159cf5,#087dcc)}
        .staff-unit-card:hover{border-color:#9ecdf2;box-shadow:0 12px 26px rgba(31,42,55,.09)}
        .staff-topic-card.selected{border-color:#159cf5;box-shadow:0 0 0 3px rgba(21,156,245,.12)}
        .flow-table thead th{background:#f4f6f8;color:#475467}
        .flow-table tbody tr:hover,.quick-actions a:hover,.document-row:hover{background:#f8fafc}
        /* One official IIUM canvas for every authenticated module. */
        body,.content-body{background-color:transparent!important}
        .content-body .container-fluid{position:relative}
        .nav-header{z-index:7!important}
        .header{z-index:6!important}
        .deznav{z-index:5!important}
        /* Official IIUM template chrome: keep one fixed navigation rail,
           a 56px teal header, the patterned canvas, and a full-width footer. */
        html,body{min-height:100%}
        body{margin:0;background:#fff!important}
        .background-image{position:fixed!important;inset:0 0 0 15rem!important;z-index:-1!important;display:block!important;background-color:#fff!important;background-repeat:repeat!important;background-position:top left!important;opacity:1!important}
        #main-wrapper{min-height:100vh!important;background:transparent!important}
        .nav-header{position:fixed!important;top:0!important;left:0!important;width:15rem!important;height:3.5rem!important;background:#fff!important;border-right:1px solid #edf0f2!important;box-shadow:none!important}
        .nav-header .brand-logo{width:100%!important;height:3.5rem!important;display:flex!important;align-items:center!important;justify-content:center!important;padding:.35rem 1rem!important}
        .nav-header .brand-logo img{width:auto!important;max-width:9.5rem!important;max-height:2.75rem!important}
        .nav-header .nav-control{display:none!important}
        .header{position:fixed!important;top:0!important;right:0!important;left:15rem!important;width:auto!important;height:3.5rem!important;padding:0!important;background:#00928f!important;box-shadow:none!important}
        .header .header-content,.header .navbar,.header .navbar-collapse{height:100%!important}
        .header .header-content{padding:0 1.25rem!important}
        .template-menu-button{width:2.5rem;height:2.5rem;display:grid;place-items:center;border:0;background:transparent;color:#fff;cursor:pointer}.template-menu-button .material-icons{font-size:1.75rem}
        .header-tools{margin-left:auto}
        .deznav{position:fixed!important;top:3.5rem!important;bottom:0!important;left:0!important;width:15rem!important;height:auto!important;padding-top:.6rem!important;background:#fff!important;border-right:1px solid #edf0f2!important;box-shadow:none!important}
        .deznav-scroll{height:100%!important;display:flex!important;flex-direction:column!important;overflow-y:auto!important}
        .deznav .metismenu{flex:0 0 auto!important;padding-bottom:1rem!important}
        .deznav .metismenu a{position:relative!important;min-height:2.875rem!important;padding:.68rem 1.625rem!important;border-radius:0!important}
        .deznav .metismenu a.active{background:#eef8f6!important;color:#159cf5!important}
        .deznav .metismenu a.active:before{height:100%!important;top:0!important;border-radius:0!important}
        .deznav .menu-title{padding:.9rem 1.625rem .55rem!important;margin:0!important;color:#159cf5!important}
        .viewer-switch{margin:auto 1rem 1rem!important}
        .content-body{min-height:calc(100vh - 7rem)!important;margin-left:15rem!important;padding-top:3.5rem!important}
        .page-titles{height:3.5rem!important;min-height:3.5rem!important;margin:0!important;padding:0 1.875rem!important;display:flex!important;align-items:center!important;background:#fff!important;border-bottom:1px solid #e3e7e9!important;border-radius:0!important;box-shadow:none!important}
        .page-titles .breadcrumb{width:100%!important}
        .content-body .container-fluid{min-height:calc(100vh - 13.25rem)!important;padding:1.875rem 1.875rem 2rem!important;background:transparent!important}
        .footer-links{margin-left:15rem;padding:.7rem 1.25rem;background:rgba(255,255,255,.88);border-top:1px solid #e1e5e7;color:#202b39;text-align:right;font-size:.8rem}
        .footer-links a{color:#202b39;text-decoration:none}.footer-links a:hover{color:#159cf5}
        .footer{margin:0 0 0 15rem!important;padding:0!important;background:#00928f!important;color:#fff!important;border:0!important;box-shadow:none!important}
        .footer .copyright{padding:1rem 1.5rem!important}.footer .copyright p{color:#fff!important;font-size:.8rem!important}
        #main-wrapper.template-nav-collapsed .nav-header,#main-wrapper.template-nav-collapsed .deznav{width:3.75rem!important}#main-wrapper.template-nav-collapsed .nav-header .brand-logo,#main-wrapper.template-nav-collapsed .deznav .nav-text,#main-wrapper.template-nav-collapsed .deznav .menu-title,#main-wrapper.template-nav-collapsed .viewer-switch{display:none!important}#main-wrapper.template-nav-collapsed .header{left:3.75rem!important}#main-wrapper.template-nav-collapsed .content-body,#main-wrapper.template-nav-collapsed .footer,#main-wrapper.template-nav-collapsed .footer-links{margin-left:3.75rem!important}#main-wrapper.template-nav-collapsed .deznav .metismenu a{justify-content:center!important;padding:.75rem 0!important}
        @media(max-width:767px){.background-image{inset:0!important}.nav-header{width:3.75rem!important}.nav-header .brand-logo{display:none!important}.header{left:3.75rem!important}.deznav{width:3.75rem!important}.deznav .nav-text,.deznav .menu-title,.viewer-switch{display:none!important}.deznav .metismenu a{justify-content:center!important;padding:.75rem 0!important}.content-body{margin-left:3.75rem!important}.footer,.footer-links{margin-left:3.75rem!important}.page-titles{padding:0 1rem!important}.content-body .container-fluid{padding:1rem!important}}
    </style>
...
@php
    $portalPageTitle = match (true) {
        ...
        request()->routeIs('organization-profile.*') => 'Organization Profile',
...
<div id="main-wrapper" class="show">
        ...
        <div class="header-content">
            <nav class="navbar navbar-expand">
                <div class="collapse navbar-collapse justify-content-between">
                    <div class="header-left"><button class="template-menu-button" id="templateMenuButton" type="button" aria-label="Toggle navigation" aria-expanded="true"><span class="material-icons">menu</span></button></div>
                    @if(request()->user())
                        <div class="header-tools">
                            ...
                            <div class="dropdown header-profile">
                                ...
                                <div class="dropdown-menu dropdown-menu-end">
                                    ...
                                    @if(request()->user()->canManagePolicies())
                                        <a class="profile-menu-link" href="{{ route('organization-profile.show') }}"><span class="material-icons">domain</span>{{ request()->user()->organizationCode() }} Profile</a>
                                    @endif
                            ...
                            <form class="header-logout" action="{{ route('staff-portal.logout') }}" method="POST">
                                @csrf
    ...
    <div class="footer-links"><a href="https://www.iium.edu.my" target="_blank" rel="noopener">IIUM Website</a> <span aria-hidden="true">||</span> <a href="https://www.iium.edu.my/disclaimer" target="_blank" rel="noopener">Disclaimers</a></div>
    <div class="footer">
        <div class="copyright">
...
<script>
    (() => {
        ...
        const menuButton = document.getElementById('templateMenuButton');
        menuButton?.addEventListener('click', () => {
            const collapsed = wrapper.classList.toggle('template-nav-collapsed');
        ...
        document.querySelector('.viewer-switch')?.addEventListener('submit', () => sessionStorage.setItem('portal-viewer-reset-scroll', '1'));
